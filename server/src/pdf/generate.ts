...
/**
 * Tech-authored amendment PDF. Deliberately minimal: red "AMENDMENT"
 * banner, back-reference to the original submission's id and timestamp,
 * verbatim comment, then a photo grid (only the amendment's photos).
 * The original submission's report.pdf is NOT re-rendered — the office
 * still has that on Splynx as a separate attachment.
 */
export async function generateAmendmentPdf(args: {
  taskId: number;
  taskAddress: string;
  ...
  return new Promise<Buffer>((resolve, reject) => {
    const doc = new PDFDocument({
      ...
      margin: 50,
      info: {
        Title: `Amendment — Task ${args.taskId} (submission ${args.originalSubmissionId})`,
        Author: "DK Connect Task Updater",
        Subject: `Amendment added ${formatDateTime(args.amendmentAddedAt)}`,
      },
    });
    ...
    const m = 50;
    // Top red accent bar so the amendment PDF is visually distinct at a
    // glance from the regular Job Completion Summary PDF.
    doc.rect(m, m - 30, doc.page.width - m * 2, 4).fill(FLAG_COLOR);
    doc.fillColor(FLAG_COLOR);
    doc.font("Helvetica-Bold").fontSize(16).text("AMENDMENT", m, m - 18, {
      width: doc.page.width - m * 2,
    });
    doc.fillColor(HEADING_COLOR);
    doc.moveDown(1.5);
    ...
    bulletLabel(doc, "Amends submission", `#${args.originalSubmissionId}`);
    bulletLabel(doc, "Task", `#${args.taskId}`);
    if (args.taskAddress.trim()) bulletLabel(doc, "Location", args.taskAddress);
    bulletLabel(doc, "Original submitted", formatDateTime(args.originalSubmittedAt));
    bulletLabel(doc, "Amendment added", formatDateTime(args.amendmentAddedAt));
    bulletLabel(doc, "Added by", args.techName);
    doc.moveDown(0.6);
    ...
    if (args.amendmentComment.trim()) {
      ...
      doc
        ...
        .text(sanitizeForPdf(args.amendmentComment), m, doc.y, {
          width: doc.page.width - m * 2,
        });
      doc.fillColor(HEADING_COLOR);
      doc.moveDown(0.6);
    ...
    if (args.photos.length > 0) {
      ...
      doc
        ...
        .text("Amendment photos");
      doc.moveDown(0.4);
      // Reuse the same grid renderer as the main report. The photo type
      // shape it needs matches exactly.
      drawPhotoGridPhotos(doc, args.photos);
    }
    ...
    doc
      ...
      .fillColor(MUTED_COLOR)
      .text(
        `Amendment generated by Task Updater  •  ${formatDateTime(
          args.amendmentAddedAt,
        )}  •  Tech: ${args.techName}`,
        m,
        footerY,
        { width: doc.page.width - m * 2, align: "center" },
...
function drawPhotoGridPhotos(
  ...
  for (let i = 0; i < photos.length; i++) {
    ...
    if (col >= cols) {
      col = 0;
      row += 1;
      if (row >= rowsPerPage && i < photos.length - 1) {
        doc.addPage();
